...
def xml_extract(xml_path, seg_label_path):
    ...
    Note:
        in the npy file, background has -1, the rest pixel value is its class
    """
    with open(xml_path) as f:
        it = itertools.chain('<root>', f, '</root>')
        root = ET.fromstringlist(it)
    image = np.ones([128, 128],dtype=np.uint8) * (-1)
    ...
    for obj in root:
        ...
        if obj.tag == 'objects':
            ...
            cv2.fillPoly( image, polygon, label)
    np.save(seg_label_path, image)
    return image_label
